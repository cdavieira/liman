...
mapa* montar_mapa(char* nome){
    ...
    if(nome){
        ...
        unsigned long tabela_ocorrencias[128] = {1, };      //armazena a frequencia de cada letra no arquivo texto
        for(char c=fgetc(fpin);c!=EOF;c=fgetc(fpin)) tabela_ocorrencias[c]++;
        listaArvores* ls = criar_listaArvores(128);
        for(long i=pegar_indice_maior_numero(tabela_ocorrencias, 128);i!=-1;i=pegar_indice_maior_numero(tabela_ocorrencias, 128)){
