...
void* pegar_conteudo_arvore(tree* arvore){
	return arvore ? (*arvore).conteudo : NULL;
}

tree* pegar_sae_arvore(tree* arvore){
	return arvore ? (*arvore).sae : NULL;
}

tree* pegar_sad_arvore(tree* arvore){
	return arvore ? (*arvore).sad : NULL;
}
...
tree* buscar_conteudo_arvore(tree* arvore, void* informacao_pesquisada, void* (*pegar_informacao_arvore)(), unsigned (*comparar)()){
	...
	if(testar_nodulo_folha(arvore)){
		if(comparar(pegar_informacao_arvore(arvore), informacao_pesquisada)){
			return arvore;
		}
	}
	else{
		if(!buscado){
			buscado = buscar_conteudo_arvore((*arvore).sae, informacao_pesquisada, pegar_informacao_arvore, comparar);
		}
		if(!buscado){
			buscado = buscar_conteudo_arvore((*arvore).sad, informacao_pesquisada, pegar_informacao_arvore, comparar);
		}
	}
	return buscado;
}
...
unsigned long contar_nodulos(tree* arvore){
	if(arvore){
		return 1 + contar_nodulos((*arvore).sae) + contar_nodulos((*arvore).sad);
	}
	return 0;
}
...
unsigned long calcular_nodulos_folha(tree* arvore){
	if(arvore){
		return testar_nodulo_folha(arvore) + calcular_nodulos_folha((*arvore).sae) + calcular_nodulos_folha((*arvore).sad);
	}
	return 0;
}
...
unsigned long calcular_altura_arvore(tree* arvore){
	if(arvore){
		return !testar_nodulo_folha(arvore) + maior(calcular_altura_arvore((*arvore).sae),calcular_altura_arvore((*arvore).sad));
	}
	return 0;
}
...
unsigned long calcular_altura_nodulo(tree* arvore, nodulo* no, unsigned long altura_inicial){
	if(arvore){
		return arvore==no?altura_inicial:(calcular_altura_nodulo((*arvore).sad, no, altura_inicial+1)+calcular_altura_nodulo((*arvore).sae, no, altura_inicial+1));
	}
	return 0;
}
...
unsigned testar_existencia_nodulo(tree* arvore, nodulo* no){
	if(arvore){
		return arvore==no?1:(testar_existencia_nodulo((*arvore).sad, no)+testar_existencia_nodulo((*arvore).sae, no));
	}
	return 0;
}
...
int comparar_conteudos_arvore(tree* arv1, tree* arv2, void* (*pegar_informacao_arvore)(), unsigned (*comparar)()){
	...
	if(comparar(pegar_informacao_arvore(arv1), pegar_informacao_arvore(arv2))){
		return 1;
	}
	return 0;
}
