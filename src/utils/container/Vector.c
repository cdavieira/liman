...
size_t vector_get_size(const Vector *arr) { return arr->size; }

size_t vector_get_capacity(const Vector *arr) { return arr->capacity; }

void *vector_get_item(Vector *arr, size_t idx) { return arr->elements[idx]; }
...
int vector_is_empty(const Vector *arr) { return !arr->size; }

int vector_is_full(const Vector *arr) { return arr->size == arr->capacity; }

static void vector_grow(Vector *arr, size_t f) {
  arr->capacity <<= f;
  arr->elements = mem_realloc(arr->elements, sizeof(void *) * arr->capacity);
  assert(arr->elements);
}

static void vector_shrink(Vector *arr, size_t f) {
  arr->capacity >>= f;
  arr->elements = mem_realloc(arr->elements, sizeof(void *) * arr->capacity);
  assert(arr->elements);
}

static int vector_is_oversized(const Vector *arr) {
  if (arr->size <= arr->min) {
    return 0;
  }
