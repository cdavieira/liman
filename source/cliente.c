...
void compactar(char* nomeArquivoEntrada){
    if(nomeArquivoEntrada){
    	char *nomeArquivoSaida = adicionar_extensao(nomeArquivoEntrada, ".comp");
        FILE* fpin = fopen(nomeArquivoEntrada, "rb"), *fpout = fopen(nomeArquivoSaida, "wb");
...
void descompactar(char* nomeArquivoBinario){
    if(nomeArquivoBinario){
    	char *nomeArquivoSaida = remover_extensao(nomeArquivoBinario);
        FILE *fpin = fopen(nomeArquivoBinario, "rb"), *fpout = fopen(nomeArquivoSaida, "wb");
        free(nomeArquivoSaida);
        assert(fpin, "Erro ao abrir arquivo compactado! Terminando programa...");
        // Debug
